...
const PeopleList = ({ chats, userIdInChats, users, setSelectedChat, scrollToBottom }) => {
    ...
    return <>
        {chats?.map((chat, index) => {
            ...
            return <>
                <button href="#" className="list-group-item list-group-item-action list-group-item-light rounded-0" key={id} onClick={() => { setSelectedChat(chats.filter((chat) => { return chat.id == id })[0]); scrollToBottom() }} >
